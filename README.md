...
docker-compose down
```
## 🧪 Разработка и тестирование (CI/CD)

В проекте настроен строгий CI/CD pipeline (GitHub Actions), который проверяет код при каждом Pull Request. Чтобы пайплайн прошел успешно, вы можете запустить проверки локально перед отправкой кода.

### Проверка форматирования (Линтер)
Проект использует встроенный в .NET линтер на основе правил файла `.editorconfig`.
Для проверки кода на соответствие стилю выполните:
```bash
dotnet format --verify-no-changes
```
> **Примечание:** Если команда выдаст ошибки, вы можете запустить dotnet format без флагов, чтобы утилита попыталась исправить их автоматически.

### Запуск тестов и проверка покрытия (Coverage)
В проекте используются xUnit и Coverlet. CI/CD пайплайн требует, чтобы покрытие кода тестами было не ниже 50%.
Для запуска тестов с подсчетом покрытия выполните команду:
```bash
